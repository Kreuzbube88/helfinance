# Reports & Auswertungen

Im Reports-Bereich analysierst du deine Finanzen — nach Monat, nach Jahr, nach Kategorie.

---

## Navigation

Unter **Reports** findest du zwei Tabs:

- **Monatsreport** — detaillierte Ansicht für einen einzelnen Monat
- **Jahresreport** — Überblick über alle 12 Monate eines Jahres

---

## Monatsreport

Wähle oben Monat und Jahr aus. Der Report zeigt:

### Einnahmen & Ausgaben-Übersicht

Eine Tabelle mit allen aktiven Buchungen im gewählten Monat:

| Einnahmen | Betrag |
|-----------|--------|
| Gehalt | 2.800€ |
| Kindergeld | 250€ |
| **Gesamt** | **3.050€** |

| Ausgaben | Betrag |
|---------|--------|
| Miete | 800€ |
| Strom/Gas | 120€ |
| Internet | 40€ |
| Lebensmittel | 380€ |
| Kreditraten | 299€ |
| **Gesamt** | **1.639€** |

| | |
|--|--|
| **Freies Geld** | **1.411€** |

### Kategorie-Donut-Chart

Ein Kreisdiagramm zeigt, wie sich deine Ausgaben auf Kategorien verteilen:

- 🔵 Wohnen: 49%
- 🟢 Lebensmittel: 23%
- 🟡 Mobilität: 15%
- 🔴 Freizeit: 13%

So siehst du auf einen Blick, wo der Großteil deines Geldes hingeht.

![Kategorie Donut Chart](../images/category-donut.png)

### Aktive Kredite

Kredite werden separat als Ausgaben-Block im Monatsreport angezeigt.

---

## Jahresreport

Der Jahresreport zeigt alle 12 Monate in einer Tabelle nebeneinander:

|  | Jan | Feb | Mär | Apr | … | Dez | **Jahr** |
|--|-----|-----|-----|-----|---|-----|---------|
| Einnahmen | 3.050 | 3.050 | 3.150 | 3.150 | … | 3.150 | **37.550** |
| Ausgaben | 1.639 | 1.639 | 1.639 | 1.639 | … | 1.639 | **19.668** |
| Frei | 1.411 | 1.411 | 1.511 | 1.511 | … | 1.511 | **17.882** |

> ✅ Im März siehst du den Effekt der Gehaltserhöhung — HELFINANCE zeigt die Planänderungen korrekt im Verlauf.

---

## Export

Beide Reports können exportiert werden:

| Format | Beschreibung |
|--------|-------------|
| **PDF** | Druckfertige Ansicht des Reports, ideal für Unterlagen |
| **CSV** | Tabellenformat für Excel, LibreOffice oder Google Sheets |

Klicke oben rechts auf **Exportieren** und wähle das Format.

---

## Filter & Zeiträume

- **Monat/Jahr wählen**: Dropdown oben im Report
- Monatsreport: Einzelner Monat
- Jahresreport: Komplettes Kalenderjahr

---

## Kategorien-Analyse

Im Monatsreport siehst du neben dem Donut-Chart auch eine Tabelle mit Kategorie-Details:

| Kategorie | Buchungen | Gesamt | Anteil |
|-----------|-----------|--------|--------|
| Wohnen | 3 | 960€ | 49% |
| Lebensmittel | 1 | 380€ | 23% |
| Mobilität | 2 | 299€ | 15% |
| Freizeit | 2 | 240€ | 13% |

**Was du daraus lernst:**
- Welche Kategorie frisst das meiste Budget?
- Wo gibt es Einsparungspotenzial?
- Welche Kategorien sind unveränderlich (Fixkosten) vs. beeinflussbar?

---

## Tipps für die Auswertung

**Monatsvergleich**

Vergleiche denselben Monat im Jahresreport mit dem Vorjahr. Haben sich Ausgaben erhöht? Neue Fixkosten dazugekommen?

**Kategorie-Overkill erkennen**

Wenn eine Kategorie mit 50%+ den Donut dominiert, lohnt es sich, diese genauer aufzuschlüsseln oder Ausgaben in dieser Kategorie zu hinterfragen.

**Jahresplanung**

Nutze den Jahresreport Ende des Jahres, um folgendes Jahr zu planen:
- Welche Ausgaben können gesenkt werden?
- Welche Verträge laufen aus?
- Wie hat sich der Health Score über das Jahr entwickelt?

---

Weiter mit: [Admin-Panel →](./08-admin.md)
